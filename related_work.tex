...
Bias is a common issue with statistical estimators, for example, the seminal work by Stein~\cite{stein81sure} fixes the bias of the mean-squared error \pl{huh? Stein estimator is the opposite - it makes estimator biased to reduce variance; here, we're debiasing}. However, debiasing an estimator does not typically lead to \emph{an improved sample complexity}, as it does in our case.

\section{Conclusion}

\pl{This paper makes three contributions:} In this paper we had three contributions: 1. We showed that the calibration error of continuous methods is underestimated; 2. We introduced the first method, to our knowledge, that has better sample complexity than histogram binning \pl{and} but has a \emph{measurable calibration error}, giving us the best of scaling and binning methods; and 3. We showed that an alternative estimator \pl{for calibration error} has better sample complexity than the plugin estimator.
...
% \end{enumerate}
There are many exciting avenues for future work, including:
\begin{enumerate}
...
\pl{All code, data, and experiments can be found on CodaLab at [link]}
