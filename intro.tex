...
\pl{not sure you really need this, because 'verify' isn't really highlighted, and it doesn't seen like a different question than anyone else is asking}
...
\pl{rewrite: As most ML..., recalibration methods are commonly used...}
Popular recalibration approaches include Platt scaling~\cite{platt1999probabilistic}, isotonic regression~\cite{zadrozny2002transforming}, and temperature scaling~\cite{guo2017calibration}. \pl{can we define a class 'scaling'...so it's clear that we're conscious about identifying a subset and not omitting histogram binning by accident} These methods are widely used, but do they actually produce calibrated probabilities?
...
Histogram binning can produce a model that is calibrated, and verifiably so \pl{not defined - what is meant by verifiably so?}, but it can be sample inefficient.
In particular, the number of samples required to calibrate scales linearly with the number of model outputs $B$~\cite{naeini2014binary}, which can be large particularly in the multi-class setting \pl{what is $B$ here?}.
Recalibration sample efficiency is crucial -- we often want to recalibrate our models in the presence of domain shift~\cite{hendrycks2019anomaly} \pl{do they actually do calibration there? if not, that's a weird cite}, and may have access to only a small labeled dataset from the target domain.
...
Prior work~\cite{nguyen2015posterior, guo2017calibration, hendrycks2019anomaly, kuleshov2015calibrated, hendrycks2019pretraining} uses the plugin \pl{say in layman's terms} estimator for the calibration error (Definition~\ref{dfn:plugin-estimator}).
Here, again, the sample complexity of the plugin estimator scales linearly with the number of model outputs $B$.
By taking advantage of error cancellations across bins, \emph{we introduce the cancelling \pl{one l} estimator} \pl{these italics look so dramatic; maybe just put it around 'canceling estimator'}.
We show that the sample complexity of our estimator scales with $\sqrt{B}$ (Theorem~\ref{thm:final-ours}).

We validate our theory \pl{this to me means the main point of the work is theory and look, we actually implemented it! just say we ran experiments} with multi-class calibration experiments on CIFAR-10~\cite{krizhevsky2009learningmultiple} and ImageNet~\cite{deng2009imagenet}.
