...
\emph{Scaling methods}, for example Platt scaling~\cite{platt1999probabilistic}, output a function $\hat{g} = \argmax_{g \in G} \sum_{(z, y) \in T} l(g(z), y)$, where $l$ \pl{use $\ell$} is a loss function, for example the log likelihood \pl{log loss} or mean-squared error. The advantage of such methods is that they converge very quickly since they only fit a single parameter.
