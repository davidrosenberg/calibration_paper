...
Suppose we have a model $f : \mathcal{X} \to [0, 1]$ where the output of the model represents the model's confidence that the label is 1. \pl{style: don't start sentence with math} $f$ may not be calibrated -- \pl{---; I'll stop bugging you} we define the calibration error, which examines the difference between the model's probability and the true probability given the model's output. If the calibration error is $0$ then the model is perfectly calibrated.
...
\pl{I suggested that we introduce multiclass early so that we can motivate how the the number of probbility outputs scales with $K$;
I think we should do that more explicitly throughout
}
...
\emph{Scaling methods}, for example Platt scaling~\cite{platt1999probabilistic}, output a function $\hat{g} = \argmin_{g \in G} \sum_{(z, y) \in T} l(g(z), y)$, where $l$ \pl{use $\ell$} is a loss function, for example the log loss or mean-squared error. The advantage of such methods is that they converge very quickly since they only fit a single parameter.
...
The bins are typically chosen such that $I_1 = [0, \frac{1}{B}], I_2 = (\frac{1}{B}, \frac{2}{B}], \dots, I_B = (\frac{B-1}{B}, 1]$ or so that each bin contains an equal number of $g(z_i)$ values in the recalibration data~\cite{zadrozny2001calibrated, guo2017calibration}. \pl{but say we don't do this...} Histogram binning then outputs the average $y_i$ value in each bin.
