...
\label{thm:final-plugin}
  Suppose \pl{use English} $\forall i.\;p_i \geq \frac{1}{2B}$ and $n = \widetilde{O}(\frac{B}{\epsilon^2})$ ignoring $\log$ factors. Then the plugin estimator can check if ${E^*}^2 \leq \epsilon^2$ with failure probability $\delta$, and constant effect size $0 < r < 1$. 
\end{restatable}

\tm{I am still not sure why it's $\widetilde{O}$ but not $\widetilde{O}$). Actually I think I haven't seen this notation $\widetilde{O}$ . }
\tm{Hmm, I think I can guess a reason. indeed this is sometimes confusing. Long story short for the moment: we should use $n = \widetilde{O}(B+\frac{\sqrt{B}}{\epsilon^2})$, unless there is a case where large $n$ would work. I know that sometimes it feels wrong to say "it requires O(d) samples to do something", but still this is the convention that people have adopted. (and there is a reasonably good reason to do it that I could explain later.)}
\begin{restatable}[Debiased bound]{theorem}{finalCanceling}
\label{thm:final-ours}
  Suppose $\forall i.\;p_i \geq \frac{1}{2B}$ and $n = \widetilde{O}(B+\frac{\sqrt{B}}{\epsilon^2})$ ignoring $\log$ factors.\tm{$\Theta(..)$ ignoring log factors sounds a quite non-standard way to say this .. First, it's unclear at the first sight why you need $\Theta$ but not $\Omega()$, second, I think often people would say $n = \widetilde{\Omega}(...)$ where $\widetilde{\Omega}$ hides log factors. Maybe you can use n gtrsim ... because gtrsim'd precise meaning could be a bit more flexible. (we don't see to have the package to support gtrsim lesssim)} Then the cancelling \pl{your names are all over the place} estimator can check if ${E^*}^2 \leq \epsilon^2$ with failure probability $\delta$ and constant $r$. 
\end{restatable}
