...
\label{thm:final-plugin}
  Suppose we have a binned model, where the binning scheme is 2-well-balanaced, that is $\forall i.\;p_i \geq \frac{1}{2B}$, and $n = \widetilde{O}(\frac{B}{\epsilon^2})$ where $\widetilde{O}$ hides $\log$ factors. Then the plugin estimator can check if ${E^*}^2 \leq \epsilon^2$ with high probability.
\end{restatable}

\begin{restatable}[Debiased bound]{theorem}{finalCanceling}
\label{thm:final-ours}
  Suppose $\forall i.\;p_i \geq \frac{1}{2B}$ and $n = \widetilde{O}(B+\frac{\sqrt{B}}{\epsilon^2})$. Then the debiased estimator can check if ${E^*}^2 \leq \epsilon^2$ with high probability.
\end{restatable}
